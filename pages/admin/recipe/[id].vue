    ...
    <v-row class="justify-center">
      <v-col cols="8" v-if="!isEdit && isOwner">
        <div class="edit-button-container text-end">
          <v-btn icon="mdi-pencil" size="small" @click="isEdit = true"></v-btn>
          <v-btn class="ms-2" icon="mdi-trash-can" size="small" @click="deleteRecipe"></v-btn>
        </div>
      </v-col>
    </v-row>
...
const isOwner = computed(
  () => recipe && recipe?.value && store.user?.id == recipe?.value.userId
);
...
const {
  ...
  error,
  refresh,
} = await useFetch(`/api/recipes/${route.params.id}`, {
...
async function deleteRecipe() {
  try {
    const { data, error } = await useFetch("/api/recipes/deleteRecipe", {
      method: "post",
      body: recipe,
    });
    ...
    console.error("Failed to delete Recipe: ", eror);
  }
}
function shareWhatsapp() {
  ...
  const ingredients = recipe.value.ingredients
    ? recipe.value.ingredients
        .map(
          (ingredient) =>
            `\n${ingredient.type}: ${ingredient.measurement} ${ingredient.unit}`
        )
        .join(", ")
    : "No Ingredients";
